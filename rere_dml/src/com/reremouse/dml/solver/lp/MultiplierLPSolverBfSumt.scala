...
/**
  ...
  */
@Deprecated
class MultiplierLPSolverBfSumt extends RereLPSolver {
    ...
    if (obj.length == const.cols && const.rows == b.length) {
      ...
      val df = new ConstrainedMathFunction() {
        override def computeObjective(x: DenseVector[Double], args: Array[Any]): Double = {
          ...
          c.t * x - u * sum(log(x)) - newV.t * (A * x - b) + (newTho / 2.0) * ((A * x - b).t * (A * x - b))+ regWeight * reg
        }
        ...
        override def computeGradient(xx: DenseVector[Double], args: Array[Any]): DenseVector[Double] = {
          ...
          val df1 = c - u / (xx) - A.t * newV + newTho * A.t * (A * xx - b)+ regWeight * reg
          df1
        }
      ...
      do {
        ...
        x0=ans
        min = re._2
        error = abs(-sum(u * log(ans)))
        //error=norm(df.computeConstrainError(ans))
        println("SUMT step:" + step + ",error:" + error + ",obj:" + min + ",u:" + u)
        u = u * 0.01
        step += 1
      } while (error > 2.0E-4 && step < maxStep)
