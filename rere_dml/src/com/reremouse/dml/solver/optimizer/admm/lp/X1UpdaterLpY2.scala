...
class X1UpdaterLpY2 extends AdmmUpdater {
  ...
  override def update(x1: DenseVector[Double],
    ...
    //val regWeight:Double = 1.0E-6
    val df = new MathFunction() {
      override def computeObjective(x: DenseVector[Double], args: Array[Any] = Array[Any]()): Double = {
        //c1.t*x  +c2.t*x2- sum(AdmmSumt.u*log(x))- sum(AdmmSumt.u*log(x2)) - lambda.t * (A1*x+A2*x2-b) + (beta/2)*(Math.pow(norm(A1*x+A2*x2-b),2))
        val reg = sum(x *:* x) * sum(x *:* x) + sum(x2 *:* x2) * sum(x2 *:* x2)
        c1.t * (x *:* x) + c2.t * (x2 *:* x2) - lambda.t * (A1 * (x *:* x) + A2 * (x2 *:* x2) - b) + (beta / 2) * norm(A1 * (x *:* x) + A2 * (x2 *:* x2) - b) + regWeight * reg
      }

      override def computeGradient(x: DenseVector[Double], args: Array[Any] = Array[Any]()): DenseVector[Double] = {
        //println("uu:"+uu)
        //val dfx = c1 - AdmmSumt.u/x -A1.t*lambda + beta*A1t *(A1*x + A2*x2 - b)
        val reg = sum(x *:* x) * x * 4.0
        val dfx = 2.0 * (c1 *:* x) - (2.0 * A1.t * lambda) *:* x + (2.0 * beta * A1.t * (A1 * (x *:* x) + A2 * (x2 *:* x2) - b) *:* x)+ regWeight * reg
        dfx
      }
    }
